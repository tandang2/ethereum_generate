...
def get_balance():
    with open("api_key.ini", "r") as key: # checks if API key in "api_key.ini"
            if key.read() == "":
                    call = urlopen(f"https://api.etherscan.io/api?module=account&action=balance&address={str(acc.address)}&tag=latest") # sends API request without key (longer time limits)
                ...
                for line in key.read():
                    call = urlopen(f"https://api.etherscan.io/api?module=account&action=balance&address={str(acc.address)}&tag=latest&apikey={str(line)}") # sends API request with given key
                    balance = load(call)["result"]
                    try:
                        return balance
                    finally:
